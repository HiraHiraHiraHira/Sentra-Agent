 ...
 */
async function _smartSendInternal(msg, response, allowReply = true) {
  // 使用内部 transport 单例
  const sendAndWaitResult = transport.sendAndWaitResult.bind(transport);
 ...
 */
export async function smartSend(msg, response, allowReply = true, options = {}) {
  ...
  return replySendQueue.enqueue(async () => {
    return await _smartSendInternal(msg, meta.response, !!meta.allowReply);
  }, taskId, meta);
}